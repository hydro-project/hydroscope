...
test.describe("Search Functionality", () => {
  ...
  test("should perform search and display results", async ({ page }) => {
    ...
    if (totalResults > 1) {
      await expect(
        page.locator('[data-testid="search-prev-button"]')
      ).toBeEnabled();
      await expect(
        page.locator('[data-testid="search-next-button"]')
      ).toBeEnabled();
    }
  });
  ...
  test("should navigate between search results", async ({ page }) => {
    ...
    if (totalResults > 1) {
      ...
      const nextResultsText = await page
        .locator('[data-testid="search-results"]')
        .textContent();
      ...
      const prevResultsText = await page
        .locator('[data-testid="search-results"]')
        .textContent();
  ...
  test("should expand containers when search finds hidden nodes", async ({
    page,
  }) => {
    ...
    const collapsedNodeCount = parseInt(
      (await page.locator('[data-testid="node-count"]').textContent()) || "0"
    );
    ...
    const expandedNodeCount = parseInt(
      (await page.locator('[data-testid="node-count"]').textContent()) || "0"
    );
  ...
  test("should clear search results and highlighting", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="graph-container"]')).toContainText(
    ...
    ).not.toContainText("Search results highlighted");
  });
  ...
  test("should maintain search state during container operations", async ({
    page,
  }) => {
    // Perform a search
    const searchTerm = "map";
    ...
    const afterExpandResultsText = await page
      .locator('[data-testid="search-results"]')
      .textContent();
    ...
    const afterCollapseResultsText = await page
      .locator('[data-testid="search-results"]')
      .textContent();
