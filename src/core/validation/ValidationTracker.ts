...
export class ValidationTracker {
  ...
  static getReport(): {
    ...
    const totalCalls = Array.from(this.callSites.values()).reduce(
  ...
  static printReport() {
    ...
    if (topPercentage > 40) {
      console.log(
        `🔥 HIGH CONCENTRATION: ${report.topCallSites[0].methods[0]} accounts for ${topPercentage}% of all validation calls`
      );
      console.log(`   → Consider reducing validation frequency in this path`);
    }
    if (report.uniqueCallSites > 25) {
      console.log(
        `🌊 HIGH CALL SITE DIVERSITY: ${report.uniqueCallSites} different locations call validation`
      );
    ...
    if (potentialRedundancy.length > 0) {
      console.log(
        `🔄 POTENTIAL REDUNDANCY: ${potentialRedundancy.length} async validation patterns detected`
      );
