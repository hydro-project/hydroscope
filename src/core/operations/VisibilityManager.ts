...
export class VisibilityManager {
  ...
  updateContainerVisibilityCaches(containerId: string, container: any): void {
    ...
    if (containerId === 'bt_40' || containerId === 'bt_204') {
      console.error(
        `[VisibilityManager] 🔍 Updating visibility cache for ${containerId}: hidden=${container.hidden}, collapsed=${container.collapsed}`
      );
    }
    ...
    if (!container.hidden) {
      ...
      if (containerId === 'bt_40' || containerId === 'bt_204') {
        console.error(
          `[VisibilityManager] 🔍 ${containerId} ADDED to visible containers (size now: ${this.state._collections._visibleContainers.size})`
        );
      }
    } else {
      ...
      if (containerId === 'bt_40' || containerId === 'bt_204') {
        console.error(
          `[VisibilityManager] 🔍 ${containerId} REMOVED from visible containers (size now: ${this.state._collections._visibleContainers.size})`
        );
        console.error(
          `[VisibilityManager] 🔍 ${containerId} REMOVAL STACK TRACE:`,
          new Error().stack
        );
      }
    }
