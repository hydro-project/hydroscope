...
describe('ELK Spacing Debug', () => {
  test('should produce reasonable spacing with our exact input data', async () => {
    ...
    console.log(
      `  Difference: ${(avgGap - expectedGap).toFixed(1)}px too ${avgGap > expectedGap ? 'wide' : 'narrow'}`
    );
    ...
    for (const config of testConfigs) {
      ...
      try {
        ...
        const testAvgGap =
          testGaps.length > 0 ? testGaps.reduce((a, b) => a + b, 0) / testGaps.length : 0;
    ...
    if (avgGap < -100) {
      console.warn(
        `⚠️ ELK produced severe overlapping (${avgGap}px gap). Consider using a different layout algorithm.`
      );
    }
