...
describe('Initial Layout Regression Tests', () => {
  ...
  test('should pass all containers as expanded to initial ELK layout call', async () => {
          ...
          children: [
            {
              id: 'container1',
              name: 'Container 1',
              children: [{ id: 'container2', name: 'Container 2', children: [] }],
            },
            {
              id: 'container3',
              name: 'Container 3',
    ...
    console.log(
      `✅ Initial ELK layout received ${expandedContainersForElk.length} expanded containers and ${visibleNodes.length} visible nodes`
    );
  });
  ...
  test('should not have empty ELK input on initial layout', async () => {
    ...
    console.log(
      `✅ ELK received ${totalElementsForElk} total elements (${expandedContainers.length} containers + ${visibleNodes.length} nodes)`
    );
  });
    ...
    const mockData = {
      ...
      nodeAssignments: {
        testGrouping: {
          ...
          ...Object.fromEntries(
            Array.from({ length: 10 }, (_, i) => [`node${i + 2}`, 'mediumContainer'])
          ),
          // Large container: 18 children (would trigger dimension explosion prevention)
          ...Object.fromEntries(
            Array.from({ length: 18 }, (_, i) => [`node${i + 12}`, 'largeContainer'])
          ),
        },
      },
    };
