...
describe('Collapsed Containers Rendering', () => {
  test('should render two collapsed containers with bidirectional hyperEdges', () => {
    ...
    console.log(
      'VisibleContainers:',
      visState.visibleContainers.map(c => ({ id: c.id, collapsed: c.collapsed, hidden: c.hidden }))
    );
    console.log(
      'VisibleNodes:',
      visState.visibleNodes.map(n => ({ id: n.id, hidden: n.hidden }))
    );
    console.log(
      'VisibleEdges:',
      visState.visibleEdges.map(e => ({
        id: e.id,
        source: e.source,
        target: e.target,
        hidden: e.hidden,
    ...
    console.log(
      'Nodes:',
      reactFlowData.nodes.map(n => ({
        id: n.id,
        type: n.type,
        position: n.position,
        data: { collapsed: n.data.collapsed, width: n.data.width, height: n.data.height },
      }))
    );
    console.log(
      'Edges:',
      reactFlowData.edges.map(e => ({ id: e.id, source: e.source, target: e.target, type: e.type }))
    );
