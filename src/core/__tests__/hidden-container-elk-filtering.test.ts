...
describe('Hidden Container ELK Filtering', () => {
  ...
  it('should exclude hidden collapsed containers from ELK layout', async () => {
    ...
    console.log(
      'Visible containers:',
      visibleContainers.map(c => ({ id: c.id, collapsed: c.collapsed, hidden: c.hidden }))
    );
  ...
  it('should not include nodes from hidden containers in visible nodes', async () => {
    ...
    console.log(
      'Visible nodes:',
      visibleNodes.map(n => ({ id: n.id, hidden: n.hidden }))
    );
  ...
  it('should validate that ELKBridge logic relies on proper filtering', () => {
    ...
    console.log(
      '✅ ELKBridge dependency on visibleContainers correctly excludes hidden containers'
    );
  });
});
