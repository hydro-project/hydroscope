...
describe('Data Integrity: Container vs Node Collections', () => {
  ...
  test('containers should not exist in graphNodes collection', () => {
    ...
    if (duplicateIds.length > 0) {
      console.error(
        `❌ Data integrity violation: ${duplicateIds.length} containers found in graphNodes collection:`
      );
  ...
  test('nodes should not exist in containers collection', () => {
    ...
    if (duplicateIds.length > 0) {
      console.error(
        `❌ Data integrity violation: ${duplicateIds.length} nodes found in containers collection:`
      );
      duplicateIds.forEach(id => {
        console.error(`  - ${id}`);
      });
  ...
  test('all entity IDs should be unique across collections', () => {
    ...
    console.log(
      `🔍 Entity counts: ${nodeIds.size} nodes, ${containerIds.size} containers, ${edgeIds.size} edges`
    );
    ...
    if (allIds.size !== totalExpected) {
      ...
      console.error(
        `❌ Data integrity violation: ${overlap} overlapping IDs detected across collections`
      );
    } else {
