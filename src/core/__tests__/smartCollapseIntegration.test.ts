  ...
  describe('Paxos-Flipped Scenario Simulation', () => {
    ...
    it('should maintain performance even with complex paxos-flipped-like datasets', async () => {
      ...
      await validateEdgeIntegrity(
        visState,
        'Post-layout after smart collapse with randomized data'
      );
  ...
  describe('Automatic Smart Collapse on Load Bug', () => {
    it('should automatically apply smart collapse during initial layout, not leave large containers expanded', async () => {
      ...
      if (buggedContainers.length > 0) {
        console.error(
          '[BUG DETECTED] Large containers not collapsed on load:',
      ...
      for (const result of results) {
        if (result.expectedCollapsed) {
          expect(
            result.actuallyCollapsed,
            `Container ${result.id} with ${result.area} pixels should be collapsed but isn't (BUG)`
          ).toBe(true);
        }
      }
      ...
      expect(
        totalVisibleArea,
        `Total visible area ${totalVisibleArea} exceeds reasonable limit ${maxReasonableArea} (dimension explosion)`
      ).toBeLessThan(maxReasonableArea);
    });
    ...
    it('🔬 DEBUG: Manual expand/collapse cycle to test hyperEdge cleanup', async () => {
      ...
      console.log(
        `🔬 Container 0 state before expansion: collapsed=${container0AfterCollapse.collapsed}`
      );

      visState.expandContainer('test_container_0');
      console.log(
        `🔬 Container 0 state after setContainer call: collapsed=${visState.getContainer('test_container_0').collapsed}`
      );
