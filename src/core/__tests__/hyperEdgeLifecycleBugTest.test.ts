...
describe('HyperEdge Lifecycle Bug Reproduction', () => {
  ...
  test('should preserve hyperEdges when multiple containers collapse sequentially', () => {
    ...
    console.log(
      'HyperEdges after loc_0 collapse:',
      hyperEdgesAfterStep1.map(e => `${e.id}: ${e.source} -> ${e.target}`)
    );
    ...
    console.log(
      'HyperEdges after loc_1 collapse:',
      hyperEdgesAfterStep2.map(e => `${e.id}: ${e.source} -> ${e.target}`)
    );
    ...
    const containerToContainerEdges = hyperEdgesAfterStep2.filter(
      edge =>
    ...
    console.log(
      'Container-to-container hyperEdges:',
      containerToContainerEdges.map(e => `${e.id}: ${e.source} -> ${e.target}`)
    );
  ...
  test('should correctly identify crossing edges including hidden ones', () => {
    ...
    console.log(
      'Crossing edges for loc_1:',
      crossingEdgesForLoc1.map(e => `${e.id}: ${e.source} -> ${e.target} (hidden: ${e.hidden})`)
    );
