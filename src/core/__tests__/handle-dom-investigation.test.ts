...
describe('Handle DOM Investigation', () => {
  test('HANDLE POSITIONING: Investigate ReactFlow internal handle positioning logic', async () => {
    ...
    console.log('  Data dimensions:', {
      width: regularNode.data.width,
      height: regularNode.data.height,
    });
    console.log('  Style dimensions:', {
      width: regularNode.style?.width,
      height: regularNode.style?.height,
    });
    ...
    console.log('  Data dimensions:', {
      width: collapsedContainer.data.width,
      height: collapsedContainer.data.height,
    });
    console.log('  Style dimensions:', {
      width: collapsedContainer.style?.width,
      height: collapsedContainer.style?.height,
    });
    ...
    console.log('  Regular node:', {
      width: regularNodeEffectiveWidth,
      height: regularNodeEffectiveHeight,
    });
    console.log('  Collapsed container:', {
      width: containerEffectiveWidth,
      height: containerEffectiveHeight,
    });
    ...
    const significantDifference =
      positionDifference.source.x > 5 ||
      positionDifference.source.y > 5 ||
      positionDifference.target.x > 5 ||
      positionDifference.target.y > 5;

    if (significantDifference) {
      console.log('🐛 SIGNIFICANT POSITION DIFFERENCE DETECTED!');
      console.log(
        '   This suggests ReactFlow is using different dimension sources for different node types'
      );
    } else {
