...
test.describe('Button Click Test', () => {
  ...
  test('should successfully click CollapseAll and verify state change', async () => {
    ...
    await page.screenshot({ path: 'hydroscope/test-results/before-button-click.png' });

    // Find and click CollapseAll button
    console.log('🔍 Looking for CollapseAll button...');
    const collapseButton = page.locator('button[title="Collapse All Containers"]').first();
    ...
    if (await collapseButton.isVisible()) {
      ...
      const afterCollapseNodes = await page.locator('.react-flow__node').count();
      const afterCollapseEdges = await page.locator('.react-flow__edge').count();
      console.log(`📊 After collapse: ${afterCollapseNodes} nodes, ${afterCollapseEdges} edges`);
      
      // Take screenshot after collapse
      await page.screenshot({ path: 'hydroscope/test-results/after-collapse-click.png' });
      ...
      if (afterCollapseNodes < initialNodes) {
        ...
        if (await fitViewButton.isVisible()) {
          ...
          await page.screenshot({ path: 'hydroscope/test-results/after-fit-view.png' });
          ...
          console.log(`📊 After fit view: ${finalNodes} nodes, ${finalEdges} edges`);
          
          // Check for floating edges by looking at edge attributes
          const edgesWithNullSource = await page.locator('.react-flow__edge[data-source="null"], .react-flow__edge:not([data-source])').count();
          const edgesWithNullTarget = await page.locator('.react-flow__edge[data-target="null"], .react-flow__edge:not([data-target])').count();
          
          console.log(`🔍 Edges with null/missing source: ${edgesWithNullSource}`);
          console.log(`🔍 Edges with null/missing target: ${edgesWithNullTarget}`);
        ...
        console.log('❌ Collapse may not have worked - node count did not decrease');
        console.log(`   Initial: ${initialNodes}, After: ${afterCollapseNodes}`);
      }
    ...
    console.log(`  - Button click successful: ${await collapseButton.isVisible()}`);
    console.log(`  - Screenshots saved: before-button-click.png, after-collapse-click.png, after-fit-view.png`);
  });
});