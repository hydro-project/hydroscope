...
test.describe("Floating Edges Debug", () => {
  ...
  test("should debug floating edges issue with real browser", async () => {
    ...
    const collapseButton = page
      .locator(
        'button:has-text("Collapse All"), button:has-text("Collapse"), [data-testid="collapse-all"], button:has-text("collapse")',
      )
      .first();
    ...
    if (await collapseButton.isVisible()) {
      ...
      console.log(
        "⚠️ Collapse All button not found, trying individual container buttons...",
      );
      // Try alternative - look for individual container collapse buttons
      const containerNodes = page.locator(
        '.react-flow__node[data-nodetype="container"]',
      );
    ...
    console.log(
      `📊 After collapse: ${collapsedNodes} nodes, ${collapsedEdges} edges`,
    );
    ...
    for (let i = 0; i < Math.min(edges.length, 20); i++) {
      // Analyze first 20 edges for detailed comparison
      const edge = edges[i];
      ...
      if (await pathElement.isVisible()) {
        ...
        // Get edge type
        const edgeType =
          (await edge.getAttribute("data-testid")) ||
          (await edge.getAttribute("class")) ||
          "unknown";
    ...
    const workingEdgesSample = edgeAnalysis.filter(
      (e) => e.hasHandles && e.hasSourceTarget,
    );
    const floatingEdgesSample = edgeAnalysis.filter(
      (e) => !e.hasHandles || !e.hasSourceTarget,
    );
    ...
    floatingEdgesSample.slice(0, 5).forEach((edge) => {
      console.log(
        `  ${edge.id}: ${edge.source || "NULL"} -> ${edge.target || "NULL"} [${edge.sourceHandle || "NULL"} -> ${edge.targetHandle || "NULL"}] (${edge.type})`,
      );
    });

    // Look for patterns
    const workingEdgeTypes = [
      ...new Set(workingEdgesSample.map((e) => e.type)),
    ];
    const floatingEdgeTypes = [
      ...new Set(floatingEdgesSample.map((e) => e.type)),
    ];
    ...
    console.log("\n🔍 EDGE ID PATTERNS:");
    console.log(
      `Working edge IDs (sample): ${workingEdgeIds.slice(0, 10).join(", ")}`,
    );
    console.log(
      `Floating edge IDs (sample): ${floatingEdgeIds.slice(0, 10).join(", ")}`,
    );

    // Check if floating edges have a pattern (e.g., all start with certain numbers)
    const floatingEdgeNumbers = floatingEdgeIds
      .map((id) => parseInt(id.replace("e", "")))
      .filter((n) => !isNaN(n));
    const workingEdgeNumbers = workingEdgeIds
      .map((id) => parseInt(id.replace("e", "")))
      .filter((n) => !isNaN(n));

    if (floatingEdgeNumbers.length > 0 && workingEdgeNumbers.length > 0) {
      console.log(
        `Floating edge number range: ${Math.min(...floatingEdgeNumbers)} - ${Math.max(...floatingEdgeNumbers)}`,
      );
      console.log(
        `Working edge number range: ${Math.min(...workingEdgeNumbers)} - ${Math.max(...workingEdgeNumbers)}`,
      );
    }
    ...
    page.on("console", (msg) => {
      const text = msg.text();
      if (
        text.includes("ReactFlowBridge") ||
        text.includes("smart handles") ||
    ...
    const reactFlowState = await page.evaluate(() => {
      ...
      if (reactFlowWrapper) {
        return {
          nodes: Array.from(document.querySelectorAll(".react-flow__node")).map(
            (node) => ({
              id: node.getAttribute("data-id"),
              type: node.getAttribute("data-nodetype"),
              position: {
                x:
                  node.style.transform?.match(/translate\(([^,]+)/)?.[1] ||
                  "unknown",
                y:
                  node.style.transform?.match(
                    /translate\([^,]+,\s*([^)]+)/,
                  )?.[1] || "unknown",
              },
            }),
          ),
          edges: Array.from(document.querySelectorAll(".react-flow__edge")).map(
            (edge) => ({
    ...
    if (floatingEdges.length > 0) {
      ...
      console.log(
        "🔍 This indicates edges are not properly connecting to handles",
      );
    }
    ...
    if (floatingEdges.length > 0) {
      console.log(
        "💡 Debug information captured. Check the screenshot and logs above.",
      );

      // Don't fail immediately - let's gather more info
      console.log(
        "🔍 Additional debugging - checking handle-edge relationships...",
      );

      // Check if handles exist for the floating edges
      for (const floatingEdgeId of floatingEdges) {
        const edge = page.locator(
          `.react-flow__edge[data-id="${floatingEdgeId}"]`,
        );
        const sourceId = await edge.getAttribute("data-source");
        const targetId = await edge.getAttribute("data-target");

        console.log(
          `🔍 Floating edge ${floatingEdgeId}: ${sourceId} -> ${targetId}`,
        );
        ...
        if (sourceExists) {
          const sourceHandles = await sourceNode
            .locator('.react-flow__handle[data-handletype="source"]')
            .count();
          console.log(`   - Source handles: ${sourceHandles}`);
        }

        if (targetExists) {
          const targetHandles = await targetNode
            .locator('.react-flow__handle[data-handletype="target"]')
            .count();
