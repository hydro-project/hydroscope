...
test.describe("Visual Floating Edges Detection", () => {
  ...
  test("should detect visual floating edges after container collapse", async () => {
    ...
    if (!collapseButton) {
      ...
      console.log(
        "⚠️ No collapse button found. Exploring custom controls area...",
      );
      ...
      for (const selector of controlsSelectors) {
        ...
        if (count > 0) {
          ...
          for (let i = 0; i < Math.min(buttonCount, 10); i++) {
            ...
            console.log(
              `    Button ${i}: title="${title}", text="${text}", visible=${isVisible}`,
            );
      ...
      if (!collapseButton) {
        console.log(
          "❌ Still no collapse button found. Listing all buttons with titles:",
        );
        ...
        for (let i = 0; i < Math.min(allButtons.length, 20); i++) {
          ...
          console.log(
            `  Button ${i}: title="${title}" (visible: ${isVisible})`,
          );
        }

        console.log(
          "❌ Cannot find collapse button. Test may not be applicable.",
        );
        return;
      }
    }
          ...
          if (pathData) {
            ...
            if (numbers.length > 0) {
              ...
              const hasExtremeCoordinates =
                minX < -1000 || maxX > 5000 || minY < -1000 || maxY > 5000;
              const hasZeroCoordinates =
                numbers.includes(0) && numbers.length > 2;
    ...
    if (suspiciousEdges.length > 0) {
      console.log(
        `❌ Found ${suspiciousEdges.length} potentially floating edges:`,
      );
      suspiciousEdges.slice(0, 5).forEach((edge, i) => {
        console.log(`  ${i + 1}. Edge ${edge.index}:`);
        console.log(`     - Path length: ${edge.pathLength.toFixed(2)}`);
        console.log(
          `     - Coordinates: X(${edge.coordinates.minX}, ${edge.coordinates.maxX}) Y(${edge.coordinates.minY}, ${edge.coordinates.maxY})`,
        );
        console.log(
          `     - Extreme coords: ${edge.hasExtremeCoordinates}, Very long: ${edge.isVeryLong}`,
        );
      });
    } else {
    ...
    console.log(
      `Found ${edgeDetails.edges.length} edges, ${edgeDetails.nodes.length} nodes, ${edgeDetails.handles.length} handles`,
    );
    ...
    suspiciousEdges.forEach((suspiciousEdge, i) => {
      ...
      console.log(
        `  - Coordinates: X(${suspiciousEdge.coordinates.minX}, ${suspiciousEdge.coordinates.maxX}) Y(${suspiciousEdge.coordinates.minY}, ${suspiciousEdge.coordinates.maxY})`,
      );

      // Find corresponding nodes
      const sourceNode = edgeDetails.nodes.find(
        (n) => n.id === edgeDetail?.source,
      );
      const targetNode = edgeDetails.nodes.find(
        (n) => n.id === edgeDetail?.target,
      );

      console.log(
        `  - Source Node: ${sourceNode ? `${sourceNode.id} (${sourceNode.type}) at ${sourceNode.position}` : "NOT FOUND"}`,
      );
      console.log(
        `  - Target Node: ${targetNode ? `${targetNode.id} (${targetNode.type}) at ${targetNode.position}` : "NOT FOUND"}`,
      );

      // Find corresponding handles
      const sourceHandles = edgeDetails.handles.filter(
        (h) => h.nodeId === edgeDetail?.source && h.type === "source",
      );
      const targetHandles = edgeDetails.handles.filter(
        (h) => h.nodeId === edgeDetail?.target && h.type === "target",
      );

      console.log(
        `  - Source Handles: ${sourceHandles.length} (${sourceHandles.map((h) => h.id).join(", ")})`,
      );
      console.log(
        `  - Target Handles: ${targetHandles.length} (${targetHandles.map((h) => h.id).join(", ")})`,
      );
    ...
    page.on("console", (msg) => {
      ...
      if (
        ...
        text.includes("FLOATING EDGE") ||
        text.includes("Edge validation")
      ) {
    ...
    const hasValidationErrors = consoleLogs.some((log) =>
      log.text.includes("FLOATING EDGE PROBLEM"),
    );

    console.log(`\n📋 Final Assessment:`);
    console.log(
      `  - Visual floating edges: ${hasVisualFloatingEdges ? "YES" : "NO"} (${suspiciousEdges.length})`,
    );
    console.log(`  - Validation errors: ${hasValidationErrors ? "YES" : "NO"}`);
    console.log(
      `  - Screenshots saved: before-collapse.png, after-collapse.png`,
    );
    ...
    if (hasVisualFloatingEdges) {
      console.log(
        `❌ Test failed: Found ${suspiciousEdges.length} visually floating edges`,
      );
      console.log(
        `💡 Check the screenshots and coordinate analysis above for details`,
      );
    } else {
