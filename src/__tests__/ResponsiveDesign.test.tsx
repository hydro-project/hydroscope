...
describe("ResponsiveDesign", () => {
  ...
  it("should set up ResizeObserver for navbar changes", async () => {
    render(<HydroscopeEnhanced responsive={true} height="600px" demo={true} />);
  ...
  it("should handle invalid URL data gracefully", async () => {
    ...
    render(<HydroscopeEnhanced enableUrlParams={true} height="600px" />);

    // Wait for error to be processed
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        "Failed to parse data parameter:",
        expect.any(Error),
      );
    });
  ...
  it("should debounce file processing", async () => {
    const { container } = render(
      <HydroscopeEnhanced height="600px" enhanced={true} />,
    );
  ...
  it("should handle style changes with debouncing", async () => {
    const { container } = render(
      <HydroscopeEnhanced height="600px" enhanced={true} demo={true} />,
    );
  ...
  it("should use static height when responsive is disabled", async () => {
    ...
    const { container } = render(
      <HydroscopeEnhanced responsive={false} height="500px" demo={true} />,
    );
