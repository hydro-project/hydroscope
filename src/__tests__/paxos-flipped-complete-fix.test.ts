...
describe("Paxos-Flipped Complete Fix", () => {
  ...
  describe("Complete Pipeline Success", () => {
    ...
    it("should demonstrate the fix prevents the original errors", async () => {
      ...
      console.log(
        "✅ Fix #2 verified: ELK layout succeeds with stress algorithm fallback",
      );

      // Verify final state - check containers since nodes might be hidden in collapsed containers
      const containersWithPositions =
        visualizationState.visibleContainers.filter((c) => c.position);
      expect(containersWithPositions.length).toBeGreaterThan(0);
      console.log(
        `✅ Final verification: ${containersWithPositions.length} containers positioned`,
      );
    });
    ...
    it("should handle the complex nested container structure correctly", async () => {
      ...
      console.log(
        `   Containers with nested containers: ${nestedContainers.length}`,
      );
      ...
      for (const container of containers) {
        ...
        expect(
          !(container.collapsed === false && container.hidden === true),
        ).toBe(true);
      ...
      console.log(
        `✅ Complex nested structure handled: ${containersWithPositions.length} containers positioned`,
      );
    });
  });
  ...
  describe("Performance and Robustness", () => {
    it("should handle paxos-flipped.json within reasonable time limits", async () => {
      ...
      // Verify results - check containers since nodes might be hidden in collapsed containers
      const containersWithPositions =
        parseResult.visualizationState.visibleContainers.filter(
          (c) => c.position,
        );

      console.log(
        `Debug: Performance test - Total visible containers: ${parseResult.visualizationState.visibleContainers.length}`,
      );
      console.log(
        `Debug: Performance test - Containers with positions: ${containersWithPositions.length}`,
      );
    ...
    it("should be deterministic - multiple runs should produce consistent results", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        results.push({
          run: i + 1,
          nodesPositioned: 0, // Not checking nodes since they might be hidden
          containersPositioned: containersWithPositions.length,
          totalNodes: 0, // Not checking nodes since they might be hidden
          totalContainers:
      ...
      for (const result of results) {
        expect(result.containersPositioned).toBe(
          firstResult.containersPositioned,
        );
      ...
      console.log(
        `   Containers positioned: ${firstResult.containersPositioned}/${firstResult.totalContainers}`,
      );
    });
  });
});
