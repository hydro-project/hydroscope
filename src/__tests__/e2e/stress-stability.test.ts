...
describe("Stress Testing for UI Operation Stability", () => {
  ...
  function setupStressTestData() {
    ...
    for (let i = 0; i < containerCount; i++) {
      const container = {
        ...
        children: new Set(
          Array.from({ length: nodesPerContainer }, (_, j) => `stress_node_${i}_${j}`)
        ),
        collapsed: false,
        hidden: false,
      };
    ...
    console.log(`Created stress test data: ${containerCount} containers, ${containerCount * nodesPerContainer} nodes, ${edgeCount} edges`);
  }
  ...
  describe("High-Frequency Container Operations", () => {
    it("should handle extremely rapid container toggles without ResizeObserver errors", async () => {
      ...
      const containerIds = Array.from({ length: 50 }, (_, i) => `stress_container_${i}`);
      ...
      console.log(`Completed ${operationCount} rapid operations in ${(endTime - startTime).toFixed(2)}ms`);
    ...
    it("should handle concurrent container operations across multiple threads", async () => {
      ...
      const concurrentPromises = Array.from({ length: concurrentBatches }, (_, batchIndex) => 
      ...
      console.log(`Completed ${concurrentBatches * operationsPerBatch} concurrent operations in ${(endTime - startTime).toFixed(2)}ms`);
  ...
  describe("Large-Scale Bulk Operations", () => {
    it("should handle massive bulk container operations", async () => {
      ...
      const allContainerIds = Array.from({ length: 50 }, (_, i) => `stress_container_${i}`);
      ...
      for (let round = 0; round < rounds; round++) {
        ...
        console.log(`Collapsed ${allContainerIds.length} containers in ${(collapseEndTime - collapseStartTime).toFixed(2)}ms`);
        ...
        console.log(`Expanded ${allContainerIds.length} containers in ${(expandEndTime - expandStartTime).toFixed(2)}ms`);
    ...
    it("should handle mixed bulk operations with different patterns", async () => {
      ...
      const mixedPatterns = [
        ...
        [
          ...containerIds.slice(0, 25).map(containerId => ({ containerId, operation: 'collapse' as const })),
          ...containerIds.slice(25).map(containerId => ({ containerId, operation: 'expand' as const }))
        ],
      ...
      for (const [patternIndex, pattern] of mixedPatterns.entries()) {
        console.log(`Executing mixed pattern ${patternIndex + 1}/${mixedPatterns.length}`);
        ...
        console.log(`Completed pattern ${patternIndex + 1} with ${pattern.length} operations in ${(endTime - startTime).toFixed(2)}ms`);
      }
  ...
  describe("High-Frequency Search Operations", () => {
    it("should handle rapid search clearing under stress", async () => {
      ...
      const mockInputRef = { current: { value: "stress test query" } as HTMLInputElement };
      ...
      console.log(`Completed ${searchOperationCount} search operations in ${(endTime - startTime).toFixed(2)}ms`);
  ...
  describe("Layout Algorithm Stress Testing", () => {
    it("should handle rapid layout algorithm changes with large datasets", async () => {
      ...
      console.log(`Completed ${layoutChangeCount} layout changes in ${(endTime - startTime).toFixed(2)}ms`);
  ...
  describe("Memory and Performance Under Stress", () => {
    it("should maintain reasonable memory usage during stress operations", async () => {
      ...
      console.log(`Stress operations completed in ${(endTime - startTime).toFixed(2)}ms`);
    ...
    it("should handle debounced operations efficiently under stress", async () => {
      ...
      const operationKeys = Array.from({ length: 10 }, (_, i) => `stress_key_${i}`);
      ...
      console.log(`Debounced stress test completed in ${(endTime - startTime).toFixed(2)}ms`);
      console.log(`Total debounced operations executed: ${totalOperations} (should be ~${operationKeys.length})`);
