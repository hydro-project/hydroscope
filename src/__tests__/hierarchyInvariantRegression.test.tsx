...
describe('Hierarchy Invariant Regression Tests', () => {
  ...
  beforeEach(async () => {
    ...
    console.log(`📦 Collapsing ${allContainers.length - topLevelContainers.length} containers, keeping ${topLevelContainers.length} top-level expanded`);
  ...
  function validateHierarchyInvariant(state: any, context: string): void {
    ...
    console.log(`🔍 [${context}] Validating hierarchy invariant for ${visibleContainers.length} visible containers`);
    ...
    for (const container of visibleContainers) {
      ...
      while (currentId && depth < 10) { // Prevent infinite loops
        ...
        if (parent) {
          ...
          if (!parentContainer) {
            throw new Error(`❌ [${context}] Container ${container.id} has parent ${parent} that doesn't exist`);
          }
          
          if (parentContainer.hidden) {
            throw new Error(`❌ [${context}] Container ${container.id} is visible but ancestor ${parent} is hidden`);
          }
          
          if (parentContainer.collapsed) {
            throw new Error(`❌ [${context}] Container ${container.id} is visible but ancestor ${parent} is collapsed`);
          }
          
          // Check that parent is also in visibleContainers
          const parentIsVisible = visibleContainers.some(c => c.id === parent);
          if (!parentIsVisible) {
            throw new Error(`❌ [${context}] Container ${container.id} is visible but ancestor ${parent} is not in visibleContainers`);
          }
        }
      ...
      if (ancestorPath.length > 0) {
        console.log(`  ✅ Container ${container.id} has valid ancestor path: ${ancestorPath.reverse().join(' -> ')} -> ${container.id}`);
      } else {
  ...
  it('should maintain hierarchy invariant during search expansion', () => {
    ...
    const currentCollapsed = new Set(visualizationState.getCollapsedContainers().map((c: any) => c.id));
    ...
    const expansionKeys = visualizationState.getSearchExpansionKeys(searchMatches, currentCollapsed);
  ...
  it('should maintain hierarchy invariant during complex search scenarios', () => {
    ...
    for (const scenario of searchScenarios) {
      ...
      const currentCollapsed = new Set(visualizationState.getCollapsedContainers().map((c: any) => c.id));
      const expansionKeys = visualizationState.getSearchExpansionKeys(scenario.matches, currentCollapsed);
