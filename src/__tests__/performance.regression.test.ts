...
describe('Performance Regression Tests', () => {
  ...
  describe('Core Component Performance', () => {
    it('should maintain JSON parsing performance', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.jsonParse);
      expect(testResult.average.memoryUsage.growth).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryGrowth);
      ...
      if (performanceBaseline?.results['json-parse']) {
        ...
        const regression = PerformanceAnalyzer.compareMetrics(
          { 
            ...
            memoryUsage: { initial: 0, peak: 0, final: 0, growth: baseline.memoryGrowth },
            throughput: baseline.throughput 
          },
          testResult.average
        );

        expect(regression.regression).toBe(false);
        
        if (regression.durationChange > 5) {
          console.warn(`JSON Parse performance regression: ${regression.durationChange.toFixed(2)}% slower`);
        }
      }

      console.log(createPerformanceReport('JSON Parse', testResult.average, {
    ...
    it('should maintain VisualizationState performance', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.containerOperations + DEFAULT_PERFORMANCE_THRESHOLDS.searchOperations);
      expect(testResult.average.memoryUsage.growth).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryGrowth);

      console.log(createPerformanceReport('VisualizationState Operations', testResult.average));
    });
    ...
    it('should maintain ELK Bridge performance', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.elkConversion);
      expect(testResult.average.memoryUsage.growth).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryGrowth);

      console.log(createPerformanceReport('ELK Bridge Conversion', testResult.average));
    });
    ...
    it('should maintain ReactFlow Bridge performance', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.reactFlowConversion);
      expect(testResult.average.memoryUsage.growth).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryGrowth);

      console.log(createPerformanceReport('ReactFlow Bridge Conversion', testResult.average));
    });
  });
  ...
  describe('Stress Testing', () => {
    it('should handle container operation stress test', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.containerOperations * 20);
      
      console.log(createPerformanceReport('Container Stress Test', testResult.average));
    });
    ...
    it('should handle search operation stress test', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.searchOperations * 20);
      
      console.log(createPerformanceReport('Search Stress Test', testResult.average));
    });
    ...
    it('should detect memory leaks in repeated operations', async () => {
      ...
      expect(testResult.average.memoryUsage.growth).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryGrowth * 2);
      ...
      expect(growthVariance).toBeLessThan(50); // Less than 50MB variance
      
      console.log(createPerformanceReport('Memory Leak Detection', testResult.average));
  ...
  describe('Synthetic Data Performance', () => {
    it('should handle large synthetic graphs efficiently', async () => {
      ...
      expect(testResult.average.duration).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.jsonParse * 2);
      expect(testResult.average.memoryUsage.peak).toBeLessThan(DEFAULT_PERFORMANCE_THRESHOLDS.memoryUsage * 2);
      
      console.log(createPerformanceReport('Large Synthetic Graph', testResult.average));
    });
  });
  ...
  describe('Throughput Testing', () => {
    it('should maintain node processing throughput', async () => {
      ...
      expect(throughput).toBeGreaterThan(DEFAULT_PERFORMANCE_THRESHOLDS.nodeProcessingThroughput);
      
      console.log(`Node Processing Throughput: ${throughput.toFixed(2)} nodes/sec (${nodeCount} nodes in ${metrics.duration.toFixed(2)}ms)`);
    });
    ...
    it('should maintain search throughput', async () => {
      ...
      expect(throughput).toBeGreaterThan(DEFAULT_PERFORMANCE_THRESHOLDS.searchThroughput);
      
      console.log(`Search Throughput: ${throughput.toFixed(2)} searches/sec (${searchCount} searches in ${metrics.duration.toFixed(2)}ms)`);
    });
  });
  ...
  describe('Performance Baseline Comparison', () => {
    it('should not regress significantly from baseline', async () => {
      if (!performanceBaseline) {
        console.log('No performance baseline available - this run will establish the baseline');
        return;
      }
      ...
      const operations = [
        ...
        },
        {
          ...
          fn: () => reactFlowBridge.toReactFlowData(parseResult.visualizationState),
        },
      ];
          ...
          const comparison = PerformanceAnalyzer.compareMetrics(
            {
              ...
              memoryUsage: { initial: 0, peak: 0, final: 0, growth: baselineResult.memoryGrowth },
