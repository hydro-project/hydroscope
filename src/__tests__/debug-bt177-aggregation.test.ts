...
describe("Debug bt_177 Aggregation", () => {
  ...
  it("should trace edge creation and resolution for bt_177", () => {
    ...
    if (bt177Container) {
      console.log(`bt_177 container hidden: ${bt177Container.hidden}, collapsed: ${bt177Container.collapsed}`);
      
      // Find the parent hierarchy
      let currentContainer = bt177Container;
      const hierarchy = [currentContainer.id];
      while (currentContainer) {
        const parentId = visualizationState.getContainerParent(currentContainer.id);
        if (parentId) {
          const parent = visualizationState.getContainer(parentId);
          if (parent) {
            hierarchy.unshift(parent.id);
            console.log(`Parent ${parent.id}: hidden=${parent.hidden}, collapsed=${parent.collapsed}`);
    ...
    const bt177Edges = aggregatedEdges.filter((edge: any) => 
      edge.source === "bt_177" || edge.target === "bt_177"
    );
    
    console.log(`Aggregated edges referencing bt_177: ${bt177Edges.length}`);
    
    for (const edge of bt177Edges) {
      console.error(`🚨 PROBLEMATIC EDGE: ${edge.id} (${edge.source} -> ${edge.target})`);
      ...
      if (bt177ContainerAfter) {
        console.error(`  bt_177 container hidden: ${bt177ContainerAfter.hidden}`);
      }
    }
  ...
  it("should not create aggregated edges referencing hidden containers during ELK layout", async () => {
    ...
    console.log(`Problematic aggregated edges before ELK: ${problematicEdges.length}`);
    
    for (const edge of problematicEdges) {
      console.error(`🚨 HIDDEN REFERENCE: ${edge.id} (${edge.source} -> ${edge.target})`);
      
      const sourceContainer = visualizationState.getContainer(edge.source);
      const targetContainer = visualizationState.getContainer(edge.target);
      
      if (sourceContainer) console.error(`  Source container ${edge.source} hidden: ${sourceContainer.hidden}`);
      if (targetContainer) console.error(`  Target container ${edge.target} hidden: ${targetContainer.hidden}`);
    }
