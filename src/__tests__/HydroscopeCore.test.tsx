...
import type { LayoutConfig, StyleConfig, ReactFlowData, LayoutState, QueueStatus } from '../types/core.js';
...
describe('HydroscopeCore Component', () => {
  ...
  describe('Ref Methods', () => {
    ...
    it('should provide access to VisualizationState', () => {
      expect(() => ref.current?.getVisualizationState()).not.toThrow();
    });

    it('should provide access to AsyncCoordinator', () => {
      expect(() => ref.current?.getAsyncCoordinator()).not.toThrow();
    });

    it('should provide access to InteractionHandler', () => {
    ...
    it('should provide updateStyleConfig method', () => {
    ...
    it('should provide clearAsyncOperations method', () => {
  ...
  describe('Error Handling', () => {
    it('should handle layout errors gracefully', async () => {
      ...
      const mockAsyncCoordinator = {
        getQueueStatus: vi.fn().mockReturnValue({
          ...
          totalProcessed: 0, averageProcessingTime: 0, errors: []
        }),
      ...
      vi.mocked(AsyncCoordinator).mockImplementationOnce(() => mockAsyncCoordinator as any);
    ...
    it('should handle ReactFlow data generation errors', async () => {
      ...
      vi.mocked(ReactFlowBridge).mockImplementationOnce(() => mockBridge as any);
  ...
  describe('Debug Mode', () => {
    it('should log debug messages when debug is enabled', async () => {
      ...
      const hasHydroscopeCoreLog = calls.some(call => 
        call[0] && typeof call[0] === 'string' && call[0].includes('[HydroscopeCore]')
      );
