  ...
  describe('JSON Parsing and Rendering with Paxos Data', () => {
    it('should successfully parse and render paxos.json data', async () => {
      ...
      await waitFor(() => {
        ...
        expect(container || screen.getByText(/loading visualization/i)).toBeTruthy();
      }, { timeout: 5000 });
    });
  ...
  describe('Bulk Operations with Paxos Data', () => {
    ...
    it('should handle individual container operations on paxos data', async () => {
      ...
      if (hydroscopeHandle) {
        ...
        const testContainerIds = ['proposers', 'acceptors', 'learners', 'clients'];
        
        for (const containerId of testContainerIds) {
          // These should not throw even if container doesn't exist
          await expect(hydroscopeHandle.collapse(containerId)).resolves.not.toThrow();
          await expect(hydroscopeHandle.expand(containerId)).resolves.not.toThrow();
          await expect(hydroscopeHandle.toggle(containerId)).resolves.not.toThrow();
        }
  ...
  describe('Error Handling with Real Data Variations', () => {
    ...
    it('should handle invalid paxos edge references', async () => {
      const invalidEdgeData = {
        ...
        edges: [
          ...paxosData.edges,
          { id: 'invalid_edge', source: 'nonexistent', target: 'alsononexistent', type: 'invalid' },
        ],
      };
