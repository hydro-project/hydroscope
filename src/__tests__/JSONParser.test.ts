  ...
  hierarchyChoices: [
    {
      ...
      name: 'Location',
      children: [
        { id: 'loc_0', name: 'hydro_test::cluster::paxos::Proposer', children: [] },
        { id: 'loc_2', name: 'hydro_test::cluster::paxos_bench::Client', children: [] }
      ]
    }
  ],
  nodeAssignments: {
...
describe('JSONParser', () => {
  ...
  describe('Node Parsing', () => {
    ...
    it('handles nodes with missing fields', async () => {
      ...
      const nodeWithGeneratedId = result.visualizationState.getGraphNode('node_1');
  ...
  describe('Node Assignment', () => {
    it('assigns nodes to containers correctly', async () => {
      ...
      const node1Container = result.visualizationState.getNodeContainer('node1');
      const node2Container = result.visualizationState.getNodeContainer('node2');
  ...
  describe('Error Handling', () => {
    ...
    it('provides detailed error context', async () => {
      ...
      const warning = result.warnings.find(w => w.type === 'edge_parsing_error');
  ...
  describe('Validation', () => {
    it('validates during parsing when enabled', async () => {
      ...
      await expect(validatingParser.parseData(simpleTestData)).resolves.toBeDefined();
    });

    it('skips validation when disabled', async () => {
      const nonValidatingParser = new JSONParser({ validateDuringParsing: false });
      
      // Should complete even with potentially invalid data
      await expect(nonValidatingParser.parseData(simpleTestData)).resolves.toBeDefined();
    });
  });
});