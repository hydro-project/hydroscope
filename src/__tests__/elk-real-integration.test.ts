...
import { createTestNode, createTestEdge, createTestContainer, loadPaxosTestData } from '../utils/testData.js'
...
describe('Real ELK Integration Tests (No Mocks)', () => {
  ...
  describe('Real ELK Layout Calculation', () => {
    it('should call real ELK library and calculate actual positions', async () => {
      ...
      const samePosition = (stateNode1!.position!.x === stateNode2!.position!.x && 
    ...
    it('should handle container layout with real ELK', async () => {
      ...
      expect(stateNode1!.position!.x).toBeGreaterThanOrEqual(stateContainer!.position!.x)
      expect(stateNode1!.position!.y).toBeGreaterThanOrEqual(stateContainer!.position!.y)
      expect(stateNode2!.position!.x).toBeGreaterThanOrEqual(stateContainer!.position!.x)
      expect(stateNode2!.position!.y).toBeGreaterThanOrEqual(stateContainer!.position!.y)
    })
  })
