...
describe('StyleProcessor', () => {
  describe('processSemanticTags', () => {
    ...
    it('should process semantic mappings correctly', () => {
      ...
      const result = processSemanticTags(['TotalOrder', 'Bounded'], styleConfig);
    ...
    it('should handle halo colors', () => {
      ...
      expect(blueResult.style.haloColor).toBe(HALO_COLOR_MAPPINGS['light-blue']);
    ...
    it('should combine multiple semantic mappings', () => {
      ...
      const result = processSemanticTags(['TotalOrder', 'Unbounded', 'Dynamic'], styleConfig);
      ...
      expect(result.appliedTags).toEqual(['TotalOrder', 'Unbounded', 'Dynamic']);
    });
    ...
    it('should combine original label with tag labels', () => {
      ...
      const result = processSemanticTags(['Network'], styleConfig, 'Original Label');
    ...
    it('should handle node styling differently from edge styling', () => {
      ...
      const edgeResult = processSemanticTags(['Critical'], styleConfig, undefined, 'edge');
      expect(edgeResult.style).toHaveProperty('strokeWidth', 3);
      expect(edgeResult.style).toHaveProperty('haloColor');

      const nodeResult = processSemanticTags(['Critical'], styleConfig, undefined, 'node');
  ...
  describe('VISUAL_CHANNELS', () => {
    ...
    it('should have correct options for each channel', () => {
      expect(VISUAL_CHANNELS['line-pattern']).toEqual(['solid', 'dashed', 'dotted', 'dash-dot']);
      ...
      expect(VISUAL_CHANNELS['halo']).toEqual(['none', 'light-blue', 'light-red', 'light-green']);
      expect(VISUAL_CHANNELS['arrowhead']).toEqual(['none', 'triangle-open', 'triangle-filled', 'circle-filled', 'diamond-open']);
