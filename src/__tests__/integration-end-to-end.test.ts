...
describe('End-to-End Integration: Complete Data Flow', () => {
  ...
  describe('Complete Pipeline: Parse → Layout → Render', () => {
    it('should process complete paxos.json pipeline successfully', () => {
      ...
      const reactFlowData = reactFlowBridge.toReactFlowData(state, interactionHandler)
    ...
    it('should handle minimal test data through complete pipeline', () => {
    ...
    function createMockELKResult(elkGraph: any) {
      ...
      if (elkGraph.children) {
        result.children = elkGraph.children.map((child: any, index: number) => ({
          id: child.id,
          x: index * 150 + 50,
          y: index * 100 + 50,
          width: child.width || 120,
          height: child.height || 60,
          children: child.children ? child.children.map((nestedChild: any, nestedIndex: number) => ({
            id: nestedChild.id,
  ...
  describe('Container Operations Through Complete Pipeline', () => {
    ...
    it('should handle container expand/collapse through complete pipeline', () => {
      ...
      elkBridge.applyLayout(state, elkResult)
      let reactFlowData = reactFlowBridge.toReactFlowData(state, interactionHandler)
      ...
      elkBridge.applyLayout(state, elkResult)
      reactFlowData = reactFlowBridge.toReactFlowData(state, interactionHandler)
    ...
    it('should handle individual container toggle through pipeline', () => {
    ...
    it('should handle click interactions through pipeline', () => {
      // Use fresh handler with debouncing disabled for synchronous testing
      const freshHandler = new InteractionHandler(state, undefined, { enableClickDebouncing: false })
      ...
      const updatedContainerNode = reactFlowData.nodes.find(node => node.id === 'c1')
  ...
  describe('Search Operations Through Complete Pipeline', () => {
    beforeEach(() => {
      ...
      const container = createTestContainer('search_container', ['search_node_1'], 'Searchable Container')
    ...
    it('should handle search operations affecting rendering', () => {
      ...
      const initialContainerNode = reactFlowData.nodes.find(node => node.id === 'search_container')
      expect(initialContainerNode?.data.collapsed).toBe(true)
      
      // Should not see internal node in collapsed state
      const hiddenNode = reactFlowData.nodes.find(node => node.id === 'search_node_1')
      ...
      const expandedContainerNode = reactFlowData.nodes.find(node => node.id === 'search_container')
      expect(expandedContainerNode?.data.collapsed).toBe(false)
      
      // Should now see the internal node
      const visibleNode = reactFlowData.nodes.find(node => node.id === 'search_node_1')
      expect(visibleNode).toBeDefined()
    })

    it('should handle search clearing through pipeline', () => {
    ...
    it('should handle search with node label highlighting', () => {
      // Perform search
  ...
  describe('Performance Validation', () => {
    it('should handle large datasets through complete pipeline efficiently', () => {
      ...
      for (let i = 0; i < containerCount; i++) {
        ...
        const container = createTestContainer(`perf_c${i}`, childNodes, `Performance Container ${i}`)
        state.addContainer(container)
    ...
    it('should handle container operations on large datasets efficiently', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const childNodes = [`bulk_n${i * 5}`, `bulk_n${i * 5 + 1}`, `bulk_n${i * 5 + 2}`]
        const container = createTestContainer(`bulk_c${i}`, childNodes, `Bulk Container ${i}`)
        state.addContainer(container)
  ...
  describe('Error Handling Through Pipeline', () => {
    it('should handle layout errors gracefully in pipeline', () => {
    ...
    it('should recover from errors in subsequent pipeline runs', () => {
      // Set up state
      const node1 = createTestNode('n1', 'Node 1')
      const node2 = createTestNode('n2', 'Node 2')
      state.addNode(node1)
      ...
      expect(node1State?.position).toEqual({ x: 100, y: 50 })
      expect(node2State?.position).toEqual({ x: 250, y: 150 })
    })
  })
})