...
import type { HydroscopeData, ValidationError, ParseError } from '../types/core.js'
...
describe('Application Integration: Complete Application with paxos.json', () => {
  ...
  describe('File Upload of paxos.json Through UI', () => {
    it('should successfully upload and process paxos.json file', async () => {
      ...
      render(
        <TestFileUpload onDataLoaded={onDataLoaded} />
      )
    ...
    it('should validate paxos.json structure', async () => {
      const invalidPaxosData = {
        nodes: [{ id: 'node1' }], // missing required properties
        edges: [] // valid but empty
      }
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(invalidPaxosData), 'invalid-paxos.json')
    ...
    it('should handle large paxos.json files efficiently', async () => {
      ...
      const largePaxosData = {
        ...
        nodeAssignments: {
          location: Object.fromEntries(
            Array.from({ length: 500 }, (_, i) => [`node_${i}`, `container_${Math.floor(i / 10)}`])
          )
        }
      }
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(largePaxosData), 'large-paxos.json')
