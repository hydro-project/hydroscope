...
describe('Error Boundary Validation', () => {
  ...
  describe('Async Operation Error Boundaries', () => {
    it('should contain async coordinator errors', async () => {
      ...
      const failingOperation = vi.fn().mockRejectedValue(new Error('Async operation failed'));
      
      // Queue the failing operation
      const promise = asyncCoordinator.queueApplicationEvent({
        type: 'test_operation',
        timestamp: Date.now()
      } as any);
      ...
      expect(asyncCoordinator.getQueueStatus().isProcessing).toBe(false);
    });
    ...
    it('should handle ELK layout errors without affecting other operations', async () => {
      ...
      // Mock ELK to fail
      const mockELK = vi.fn().mockRejectedValue(new Error('ELK layout failed'));

      // Queue layout operation
      const layoutPromise = asyncCoordinator.queueELKLayout(visualizationState, {
        algorithm: 'layered',
        direction: 'DOWN',
      ...
      await expect(layoutPromise).rejects.toThrow('ELK layout failed');
      ...
      await expect(appEventPromise).resolves.toBeUndefined();
    });
    ...
    it('should handle ReactFlow render errors without affecting state', async () => {
      ...
      // Mock ReactFlow conversion to fail
      const renderPromise = asyncCoordinator.queueReactFlowRender(visualizationState);
  ...
  describe('Data Integrity Error Boundaries', () => {
    ...
    it('should handle edge reference corruption', () => {
      ...
      const internalEdges = (visualizationState as any).edges;
      ...
      expect(() => {
        visualizationState.validateInvariants();
      }).toThrow('Edge references non-existent node');
    });
    ...
    it('should handle container hierarchy corruption', () => {
      ...
      // Validation should detect orphaned references
      expect(() => {
        visualizationState.validateInvariants();
      }).toThrow('Container references non-existent child');
    });
  });
  ...
  describe('Resource Exhaustion Error Boundaries', () => {
    it('should handle memory exhaustion gracefully', () => {
      ...
      // Try to create a very large graph
      ...
      try {
        for (let i = 0; i < nodeCount; i++) {
          visualizationState.addNode({
            id: `node${i}`,
            label: `Node ${i}`,
            longLabel: `Node ${i} Long Label with lots of text to consume memory`,
            type: 'process',
            semanticTags: [`tag${i}`, `category${i % 100}`, `group${i % 10}`],
            hidden: false
          });
          addedNodes++;
        }
