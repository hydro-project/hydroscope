...
describe('File Loading Regression Tests', () => {
  ...
  it('should reload visualization when a different file is loaded via file input', async () => {
    ...
    const mockFile = new File([JSON.stringify(newData)], 'new-data.json', { type: 'application/json' });
    const mockEvent = {
    ...
    await waitFor(() => {
      ...
      expect(mockParseData.mock.calls.length).toBeGreaterThan(initialCallCount);
    }, { timeout: 3000 });

    // Check if the new data was processed
    const allCalls = mockParseData.mock.calls;
    const hasNewDataCall = allCalls.some(call => 
      call[0] && 
      call[0].nodes && 
  ...
  it.skip('should handle file loading errors gracefully', async () => {
    ...
    const loadFileButton = screen.getAllByTestId('control-button').find(button => 
      button.getAttribute('title')?.includes('Load another file')
    );
    ...
    mockInput.onchange({ target: { files: [new File(['invalid json'], 'invalid.json')] } });
 ...
 it.skip('should call handleFileLoaded when file is selected', async () => {
    ...
    const TestComponent = () => {
      ...
      const handleFileLoaded = React.useCallback((newData: HydroscopeData, filename: string) => {
