...
describe("Unified Data Processing Pipeline", () => {
  ...
  async function simulateUnifiedPipeline(
    ...
    state.performSmartCollapse = function(budgetOverride?: number) {
      ...
      console.log(`🎯 SMART COLLAPSE CALLED for ${reason} (call #${smartCollapseCallCount})`);
    ...
    console.log(`🔄 Reset layout state for ${reason} - smart collapse will run on next layout`);
  ...
  it("should run smart collapse for hierarchy change", async () => {
    ...
    const result = await simulateUnifiedPipeline(hierarchyChangedData, 'hierarchy_change');
  ...
  it("should NOT run smart collapse without layout state reset", async () => {
    ...
    const newData = {
      ...data,
      nodes: [...data.nodes, { id: "node11", label: "Node 11", type: "default" }]
    };
    ...
    console.log('❌ BUG DEMONSTRATED: Smart collapse did not run without layout state reset');
  });
});