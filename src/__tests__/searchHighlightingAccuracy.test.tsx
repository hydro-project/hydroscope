...
describe('Search Highlighting Accuracy', () => {
    ...
    beforeEach(async () => {
        ...
        const paxosJsonString = readFileSync(
            require.resolve('../test-data/paxos.json'),
            'utf-8'
        );
        ...
        console.log('Available hierarchy choices:', hierarchyChoices.map(c => c.name));
    ...
    it('should only highlight actual search matches, not expanded containers', async () => {
        ...
        const uniqueContainers = allContainers.filter((container, index, arr) =>
        ...
        console.log(`Search matches for "${query}":`, searchMatches.map(m => `${m.id} (${m.type}): "${m.label}"`));
        ...
        const filteredMatches = searchMatches.filter(m => m && (m.type === 'node' || m.type === 'container'));
        ...
        console.log(`Items that would be highlighted:`, highlightedItems.map(item => `${item.id} (${item.type}): "${item.label}"`));
        ...
        for (const item of highlightedItems) {
            ...
            if (searchMatch) {
                ...
                console.log(`✅ Correctly highlighted search match: ${item.id} (${item.type}) - "${item.label}"`);
            } else {
                // This item is highlighted but not in search matches - this is the bug!
                console.error(`❌ BUG: Item ${item.id} (${item.type}) with label "${item.label}" would be highlighted but is not in search matches`);

                if (item.type === 'container') {
                    console.error(`❌ Container "${item.label}" (${item.id}) should NOT be highlighted just because it contains matching nodes`);
                }
    ...
    it('should not highlight containers with non-matching labels even if they contain matching nodes', async () => {
        ...
        if (suspiciousContainers.length > 0) {
            ...
            suspiciousContainers.forEach(container => {
                ...
                if (inMatches) {
                    console.error(`❌ BUG FOUND: Container ${container.id} with label "${label}" is in search matches for "leader"`);
        ...
        for (const match of searchMatches) {
            if (!match.label.toLowerCase().includes('leader')) {
                console.error(`❌ BUG: Match ${match.id} (${match.type}) with label "${match.label}" doesn't contain "leader"`);
                expect(match.label.toLowerCase()).toContain('leader');
            }
        }
    });
    ...
    it('should find items with leader in their labels (nodes or containers)', async () => {
        ...
        console.log(`Search expansion would expand ${searchExpansionKeys.length} containers:`, searchExpansionKeys);
        ...
        for (const key of searchExpansionKeys) {
            ...
            if (containerInMatches && !containerLabelHasLeader) {
                console.error(`❌ BUG: Container ${key} with label "${containerLabel}" is in search matches but doesn't contain "leader"`);
  ...
  it('should reproduce the sequence_payload highlighting bug from screenshot', async () => {
    ...
    const sequencePayloadContainers = searchableItems.filter(item => 
      item.type === 'container' && item.label.includes('sequence_payload')
    );
    ...
    if (sequencePayloadInMatches.length > 0) {
      console.error(`❌ BUG REPRODUCED: Found sequence_payload containers in search matches for "leader":`);
      ...
      console.log('✅ No sequence_payload containers found in search matches - bug not reproduced in this test');
    }
    ...
    console.log(`🎨 Checking highlighting logic for ${visibleContainers.length} visible containers...`);
    ...
    for (const container of visibleContainers) {
      ...
      if (isMatch) {
        ...
        if (!containerLabel.toLowerCase().includes(query.toLowerCase())) {
          console.error(`❌ BUG FOUND: Container ${container.id} with label "${containerLabel}" would be highlighted but doesn't contain "${query}"`);
      ...
      if (containerLabel.includes('sequence_payload') && isMatch) {
        console.error(`❌ BUG CONFIRMED: sequence_payload container ${container.id} ("${containerLabel}") would be highlighted when searching for "${query}"`);
  ...
  it('should test FlowGraph highlighting logic directly', async () => {
    ...
    const filteredMatches = mockSearchMatches.filter(m => m && (m.type === 'node' || m.type === 'container'));
    const matchSet = new Set(filteredMatches.map(m => m.id));
    ...
    for (const node of mockNodes) {
      ...
      if (isMatch) {
        ...
        if (!nodeLabel.toLowerCase().includes(searchQuery.toLowerCase())) {
          ...
          console.error(`❌ BUG: Node ${node.id} with label "${nodeLabel}" would be highlighted but doesn't contain "${searchQuery}"`);
        }
      } else {
    ...
    console.log(`✅ Correctly highlighted nodes: ${highlightedNodes.length - incorrectlyHighlightedNodes.length}`);
  ...
  it('should check for z-order and nested container highlighting issues', async () => {
    ...
    for (const container of sequencePayloadContainers) {
      ...
      if (leaderAncestors.length > 0) {
        console.log(`🔍 POTENTIAL BUG SOURCE: sequence_payload container ${container.id} has ancestors with "leader":`);
        ...
        console.log(`💡 THEORY: Parent container "${leaderAncestors[0].label}" gets highlighted, but due to z-order/layering, it appears as if "${label}" is highlighted`);
      }
    }
    ...
    for (const sequenceContainer of sequencePayloadContainers) {
      const containerLabel = sequenceContainer?.data?.label || sequenceContainer?.label || sequenceContainer.id;
      ...
      while (currentId && level < 10) {
        ...
        if (parent) {
          ...
          if (parentInMatches) {
            ...
            console.log(`  - sequence_payload container: ${sequenceContainer.id} ("${containerLabel}")`);
            console.log(`  - Parent container in search matches: ${parent} ("${parentInMatches.label}")`);
            console.log(`  - This parent's highlight might visually appear to belong to the child due to layering!`);
            
            // This is likely the root cause of the bug!
            console.log(`❌ BUG EXPLANATION: The parent container "${parentInMatches.label}" is correctly highlighted, but due to CSS layering/z-index issues, the highlight appears to belong to the child container "${containerLabel}"`);
          }
          currentId = parent;
          level++;
        } else {
  ...
  it('should verify ContainerNode z-index fix for search highlighting', () => {
    ...
    const getExpectedZIndex = (data: any) => {
      return data.searchHighlightStrong
        ? 30
    ...
    console.log('✅ Z-index hierarchy is correct - highlighted containers will appear above normal ones');
    console.log('🎯 This should fix the visual layering bug where highlight borders appear to belong to wrong containers');
  });
});