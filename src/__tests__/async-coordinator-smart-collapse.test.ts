...
describe("AsyncCoordinator Smart Collapse Integration", () => {
  ...
  it("should run smart collapse through AsyncCoordinator pipeline on initialization", async () => {
    ...
    console.log('🧪 Testing smart collapse through AsyncCoordinator pipeline...');
    ...
    state.performSmartCollapse = function(budgetOverride?: number) {
      console.log('🎯 SMART COLLAPSE CALLED THROUGH ASYNC COORDINATOR PIPELINE!');
    ...
    console.log(`  Container1 (10 children): collapsed=${container1Before?.collapsed}`);
    console.log(`  Container2 (3 children): collapsed=${container2Before?.collapsed}`);
    ...
    console.log(`  Container1 (10 children): collapsed=${container1After?.collapsed}`);
    console.log(`  Container2 (3 children): collapsed=${container2After?.collapsed}`);
    ...
    console.log('🎉 SUCCESS: Smart collapse worked correctly through AsyncCoordinator pipeline!');
  });
  ...
  it("should not run smart collapse on subsequent AsyncCoordinator operations", async () => {
    ...
    console.log('🔄 Running second AsyncCoordinator pipeline (should NOT trigger smart collapse)...');
    await asyncCoordinator.executeLayoutAndRenderPipeline(state, { fitView: false });
    ...
    console.log('✅ SUCCESS: Smart collapse correctly skipped on subsequent AsyncCoordinator operations!');
  });
});