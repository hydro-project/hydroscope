...
let _resizeObserverCallCount = 0;
const mockResizeObserver = vi.fn(() => ({
  observe: vi.fn(() => {
    _resizeObserverCallCount++;
  }),
  unobserve: vi.fn(),
  disconnect: vi.fn(),
}));
...
describe('Comprehensive ResizeObserver Loop Prevention', () => {
  ...
  describe.skipIf(SKIP_OLD_SYSTEM_TESTS)('Global ReactFlow Operation Manager', () => {
    ...
    it('should batch (and debounce) fitView operations', async () => {
      ...
      const fitOp1 = globalReactFlowOperationManager.fitView(mockFitView, { padding: 0.1 }, 'high');
  ...
  describe.skipIf(SKIP_OLD_SYSTEM_TESTS)('Integration: Error Prevention', () => {
    it('should not generate ResizeObserver loop errors during operations', async () => {
      ...
      const operations = [
        ...
        globalReactFlowOperationManager.fitView(vi.fn(), { padding: 0.1 }, 'normal'),
        globalReactFlowOperationManager.updateSearchHighlighting(vi.fn(), 'normal'),
      ];
