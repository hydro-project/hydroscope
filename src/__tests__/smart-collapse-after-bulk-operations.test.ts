...
describe("Smart Collapse After Bulk Operations Regression", () => {
  ...
  it("should run smart collapse on new file load even after user performed collapseAll", async () => {
    ...
    const chatContainer1 = createTestContainer(
      "chat_container1",
      "Chat Container 1",
      60,
    ); // Large
    const chatContainer2 = createTestContainer(
      "chat_container2",
      "Chat Container 2",
      5,
    ); // Small
    ...
    // Add child nodes
    [
      ...Array.from(chatContainer1.children),
      ...Array.from(chatContainer2.children),
    ].forEach((nodeId) => {
    ...
    const paxosContainer1 = createTestContainer(
      "paxos_container1",
      "Paxos Container 1",
      80,
    ); // Large
    const paxosContainer2 = createTestContainer(
      "paxos_container2",
      "Paxos Container 2",
      3,
    ); // Small
    ...
    // Add child nodes
    [
      ...Array.from(paxosContainer1.children),
      ...Array.from(paxosContainer2.children),
    ].forEach((nodeId) => {
      state.addNode(createTestNode(nodeId, `Node ${nodeId}`));
    });
    ...
    expect(state.shouldRunSmartCollapse()).toBe(true);
    console.log(
      "✅ Smart collapse re-enabled for new data (this was the bug fix)",
    );
    ...
    if (state.shouldRunSmartCollapse()) {
      ...
      containers.forEach((container) => {
        if (container.children.size > 50) {
          ...
          console.log(
            `🎯 Smart collapse: Collapsed large container ${container.id} (${container.children.size} children)`,
          );
        }
      });
    }
    ...
    console.log(
      "🎉 SUCCESS: Smart collapse worked correctly after bulk operations!",
    );
    console.log(
      `   - Large container (${paxosContainer1After?.children.size} children): ${paxosContainer1After?.collapsed ? "COLLAPSED" : "EXPANDED"}`,
    );
    console.log(
      `   - Small container (${paxosContainer2After?.children.size} children): ${paxosContainer2After?.collapsed ? "COLLAPSED" : "EXPANDED"}`,
    );
  });
  ...
  it("should maintain smart collapse behavior across multiple file loads", async () => {
    ...
    console.log(
      "🎉 SUCCESS: Smart collapse works correctly across multiple file loads",
    );
  });
});
