...
describe("Container State Invariant Bug", () => {
  ...
  describe("VisualizationState Invariant Violations", () => {
    it("should identify containers in illegal Expanded/Hidden state", async () => {
      ...
      console.log(`Found ${illegalContainers.length} containers in illegal Expanded/Hidden state:`);
      illegalContainers.forEach(id => console.log(`  - ${id}`));

      console.log(`Found ${ancestorViolations.length} ancestor collapse violations:`);
      ancestorViolations.forEach(violation => console.log(`  - ${violation}`));

      // After the fix, there should be no invariant violations
      console.log(`✅ Fix verification: ${illegalContainers.length} illegal containers, ${ancestorViolations.length} ancestor violations`);
    ...
    it("should now succeed with ELK layout after fix", async () => {
      ...
      try {
        ...
        console.log("✅ ELK layout succeeded after fixing invariant violations");
        ...
        expect(errorMessage).not.toContain("VisualizationState invariant violations");
        ...
        if (errorMessage.includes("Invalid hitboxes") || errorMessage.includes("ELK layout calculation failed")) {
          console.log("✅ Invariant violation bug is fixed, but there may be other ELK issues to address");
        } else {
    ...
    it("should identify the problematic container hierarchy", async () => {
      ...
      const ancestorContainer = visualizationState.visibleContainers.find(c => c.id === problematicAncestor);
      ...
      if (ancestorContainer) {
        ...
        const descendants = findAllDescendants(problematicAncestor, containerHierarchy);
        ...
        const illegalDescendants = descendants.filter(descendantId => {
          const descendant = visualizationState.visibleContainers.find(c => c.id === descendantId);
          return descendant && !descendant.collapsed && ancestorContainer.collapsed;
        });
        
        console.log(`  - Illegal descendants (should be collapsed): ${illegalDescendants.length}`);
 ...
 */
function buildContainerHierarchy(state: VisualizationState): Map<string, string[]> {
 ...
 */
function findAllDescendants(containerId: string, hierarchy: Map<string, string[]>): string[] {
