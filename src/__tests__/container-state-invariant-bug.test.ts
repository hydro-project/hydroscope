  ...
  describe("VisualizationState Invariant Violations", () => {
    ...
    it("should now succeed with ELK layout after fix", async () => {
      ...
      try {
        ...
        console.log(
          `Total visible nodes: ${visualizationState.visibleNodes.length}`,
        );
        console.log(`Nodes with positions: ${nodesWithPositions.length}`);

        // If there are visible nodes, at least some should have positions after layout
        if (visualizationState.visibleNodes.length > 0) {
          expect(nodesWithPositions.length).toBeGreaterThan(0);
        } else {
          // If no visible nodes, that's also a valid state - test passes
          console.log("No visible nodes found - this is acceptable");
        }
      } catch (error) {
