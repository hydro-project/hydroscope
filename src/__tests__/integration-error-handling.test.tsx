...
import { HydroscopeCore, type HydroscopeCoreRef } from '../components/HydroscopeCore.js'
import { FileUpload } from '../components/FileUpload.js'
import { ContainerControls } from '../components/ContainerControls.js'
import { Search } from '../components/Search.js'
import type { HydroscopeData, LayoutState, QueueStatus, ValidationError, ParseError } from '../types/core.js'
...
const ErrorTestApplication: React.FC<{
  ...
  const handleFileLoaded = React.useCallback((data: HydroscopeData, filename: string) => {
    try {
      ...
      setErrorMessage(`Data loading failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      onError?.(error instanceof Error ? error : new Error('Unknown error'), 'data loading')
    }
  }, [onError])

  const handleValidationError = React.useCallback((errors: ValidationError[], filename: string) => {
    const errorMsg = `Validation failed for ${filename}: ${errors.map(e => e.message).join(', ')}`
    setErrorMessage(errorMsg)
    onValidationError?.(errors, filename)
  }, [onValidationError])

  const handleParseError = React.useCallback((error: ParseError, filename: string) => {
    const errorMsg = `Parse error in ${filename}: ${error.message}`
    setErrorMessage(errorMsg)
    onParseError?.(error, filename)
  }, [onParseError])

  const handleCoreError = React.useCallback((error: Error, context: string) => {
      ...
      {isDataLoaded && (
        <div data-testid="controls-section">
          <ContainerControls
            onExpandAll={() => {
              try {
                coreRef.current?.getVisualizationState()?.expandAllContainers()
                coreRef.current?.triggerLayout()
              } catch (error) {
                handleCoreError(error instanceof Error ? error : new Error('Unknown error'), 'expand all')
              }
            }}
            onCollapseAll={() => {
              try {
                coreRef.current?.getVisualizationState()?.collapseAllContainers()
                coreRef.current?.triggerLayout()
              } catch (error) {
                handleCoreError(error instanceof Error ? error : new Error('Unknown error'), 'collapse all')
              }
            }}
          ...
          <Search
            onSearch={(query) => {
              try {
                ...
                handleCoreError(error instanceof Error ? error : new Error('Unknown error'), 'search')
              }
            }}
            data-testid="search"
          />
...
describe('Application Integration: Error Handling and Edge Cases', () => {
  ...
  describe('Invalid File Upload Scenarios', () => {
    it('should handle completely invalid JSON files', async () => {
      ...
      const file = createMockFile('{ this is not valid json at all }', 'invalid.json')
      ...
      expect(screen.getByText(/Parse error in invalid.json/)).toBeInTheDocument()
    })
    ...
    it('should handle files with wrong MIME types', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile('valid json content', 'test.txt', 'text/plain')
  ...
  describe('Corrupted Data Scenarios', () => {
    it('should handle JSON with missing required properties', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'corrupted.json')
    ...
    it('should handle nodes with invalid structure', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'bad-nodes.json')
    ...
    it('should handle edges with invalid references', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'bad-edges.json')
    ...
    it('should handle circular references in hierarchy', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'circular.json')
    ...
    it('should handle malformed Unicode characters', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'unicode.json')
  ...
  describe('UI Responsiveness Under High Load', () => {
    it('should remain responsive during rapid file uploads', async () => {
      ...
      render(<ErrorTestApplication onError={onError} onValidationError={onValidationError} />)
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const file = createMockFile(JSON.stringify(validData), `rapid-${i}.json`)
    ...
    it('should handle stress testing with large corrupted files', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(largeCorruptedData), 'large-corrupted.json')
    ...
    it('should handle memory pressure gracefully', async () => {
      ...
      for (let i = 0; i < memoryPressureData.length; i++) {
        ...
        const file = createMockFile(JSON.stringify(memoryPressureData[i]), `memory-${i}.json`)
  ...
  describe('Error Messages and User Feedback', () => {
    it('should display clear error messages for different error types', async () => {
      ...
      for (const scenario of errorScenarios) {
        ...
        const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
        const file = createMockFile(scenario.content, `${scenario.name.toLowerCase().replace(/\s+/g, '-')}.json`)
    ...
    it('should provide actionable error messages', async () => {
      const corruptedData = {
        nodes: [
          { /* missing id */ label: 'Node without ID' }
        ],
        edges: []
      }
      setupFileReaderMock(JSON.stringify(corruptedData))
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(corruptedData), 'actionable-error.json')
    ...
    it('should handle multiple simultaneous errors', async () => {
      ...
      const fileInput = screen.getByRole('button').querySelector('input[type="file"]') as HTMLInputElement
      const file = createMockFile(JSON.stringify(multiErrorData), 'multi-error.json')
  ...
  describe('Component Error Boundaries', () => {
    it('should handle core component initialization failures', async () => {
      ...
      render(
        <ErrorTestApplication onError={onError} />
      )
