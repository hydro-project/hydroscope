...
describe('ELKBridge', () => {
  ...
  describe('layoutVisualizationState', () => {
    ...
    it('should ensure children are positioned within parent containers', async () => {
      ...
      console.log(
        `✅ Container hierarchy test: ${children.length} children positioned within container bounds`
      );
    });
    ...
    it('should preserve and store edge sections from ELK layout', async () => {
      ...
      if (edgeLayout?.sections && edgeLayout.sections.length > 0) {
        ...
        console.log(
          `ℹ️  Edge ${testEdge.id} has no sections (may be cross-container or simple direct connection)`
        );
      }
