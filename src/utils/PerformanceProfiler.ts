...
export class ComponentProfiler {
  ...
  private operationStack: Array<{ name: string; startTime: number; startMemory: number }> = [];
  ...
  endOperation(metadata?: Record<string, any>): ProfileResult | null {
    ...
    const result: ProfileResult = {
      ...
      metadata,
      recommendations: this.generateRecommendations(operation.name, endTime - operation.startTime, endMemory - operation.startMemory),
    };
  ...
  private generateRecommendations(operationName: string, duration: number, memoryGrowth: number): string[] {
    const recommendations: string[] = [];

    // Duration-based recommendations
    if (duration > 100) {
      recommendations.push(`Operation ${operationName} took ${duration.toFixed(2)}ms - consider optimization`);
      
      if (operationName.includes('search')) {
        recommendations.push('Consider implementing search result caching or indexing');
      }
      
      if (operationName.includes('elk') || operationName.includes('layout')) {
        recommendations.push('Consider layout result caching for unchanged graphs');
      }
      
      if (operationName.includes('reactflow') || operationName.includes('render')) {
        recommendations.push('Consider memoization of ReactFlow node/edge generation');
      }
    }

    // Memory-based recommendations
    if (memoryGrowth > 10) { // 10MB growth
      recommendations.push(`Operation ${operationName} used ${memoryGrowth.toFixed(2)}MB - check for memory leaks`);
  ...
  private generateSummary(session: ProfileSession): ProfileSession['summary'] {
    ...
    const totalMemoryGrowth = session.results.reduce((sum, r) => sum + r.memoryUsage.growth, 0);
...
export function profile(operationName?: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    const profileName = operationName || `${target.constructor.name}.${propertyKey}`;
    ...
    descriptor.value = function (...args: any[]) {
      ...
      try {
        ...
        globalProfiler.endOperation({ error: error.message });
        throw error;
      }
    };
...
export function profileFunction<T>(
  ...
  try {
    ...
    globalProfiler.endOperation({ error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}
