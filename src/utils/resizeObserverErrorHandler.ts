...
export class ResizeObserverErrorHandler {
                ...
                try {
                    ...
                    if (text.includes('ResizeObserver loop') && text.length < 2000) {
                        ...
                        if (container && container.style) {
                            ...
                            console.info('[ResizeObserverErrorHandler] Removed existing ResizeObserver-only overlay');
                        }
                    }
                } catch { /* ignore */ }
            });
    ...
    private recordSuppression() {
        ...
        if (this.suppressedCount <= 3 || this.suppressedCount % 10 === 0) {
            console.info(`[ResizeObserver] Prevented loop error #${this.suppressedCount} (recent=${this.recentSuppressTimestamps.length})`);
        }
        if (!this.paused && this.recentSuppressTimestamps.length >= this.watchdogThreshold) {
            console.warn('[ResizeObserverErrorHandler] Watchdog: rapid suppressions detected – pausing ResizeObserver temporarily');
            this.pause();
