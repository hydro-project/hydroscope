...
describe('OperationPerformanceMonitor', () => {
  ...
  describe('Cascade Detection', () => {
    it('should detect cascades when operations exceed threshold', () => {
      const operations: OperationType[] = ['container_toggle', 'container_expand', 'container_collapse', 'panel_toggle'];
      ...
      expect(cascadeDetection.recommendations).toContain('Consider batching multiple operations together');
    });
    ...
    it('should provide specific recommendations based on operation types', () => {
      ...
      ['container_toggle', 'container_expand', 'container_collapse'].forEach(op => {
        monitor.startOperation(op as OperationType);
        monitor.endOperation(op as OperationType);
      });
      
      const cascadeDetection = monitor.getCascadeDetection();
      expect(cascadeDetection.recommendations).toContain('Use batchContainerOperationsImperatively() for multiple container operations');
    });
  });
  ...
  describe('Performance Report Generation', () => {
    it('should generate comprehensive performance report', () => {
      ...
      ['container_toggle', 'container_expand', 'container_collapse', 'panel_toggle'].forEach(op => {
        monitor.startOperation(op as OperationType);
        monitor.endOperation(op as OperationType);
      });
...
describe('Global Operation Monitor', () => {
  ...
  beforeEach(() => {
    ...
    (globalOperationMonitor as any).config = { ...originalConfig, enabled: true };
...
describe('Utility Functions', () => {
  ...
  describe('monitorOperation', () => {
    it('should monitor synchronous operations', () => {
      ...
      const history = globalOperationMonitor.getOperationHistory('panel_toggle');
  ...
  describe('monitorAsyncOperation', () => {
    it('should monitor asynchronous operations', async () => {
      ...
      const result = await monitorAsyncOperation('style_layout', testFn, { async: true });
      
      expect(result).toBe('async result');
      expect(testFn).toHaveBeenCalledOnce();
      
      const history = globalOperationMonitor.getOperationHistory('style_layout');
      expect(history).toHaveLength(1);
  ...
  describe('measureOperationPerformance', () => {
    it('should measure and return operation metrics', () => {
      ...
      const { result, metrics } = measureOperationPerformance('container_batch', testFn, { measure: true });
  ...
  describe('measureAsyncOperationPerformance', () => {
    it('should measure and return async operation metrics', async () => {
      ...
      const { result, metrics } = await measureAsyncOperationPerformance('style_batch', testFn, { asyncMeasure: true });
...
describe('Performance Thresholds', () => {
  ...
  beforeEach(() => {
    ...
    monitor = new OperationPerformanceMonitor({
      enabled: true,
      thresholds: {
        container_toggle: { maxDuration: 1, maxMemoryGrowth: 0.1, maxCoordinatorCalls: 0 }
      }
    });
  });
