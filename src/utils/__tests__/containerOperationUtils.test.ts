...
describe("Container Operation Utilities", () => {
  ...
  describe("toggleContainerImperatively", () => {
    it("should expand a collapsed container", () => {
      ...
      expect(mockVisualizationState._expandContainerForCoordinator).toHaveBeenCalledWith("container1");
      expect(mockVisualizationState._collapseContainerForCoordinator).not.toHaveBeenCalled();
    });
    ...
    it("should collapse an expanded container", () => {
      ...
      // Verify
      expect(result).toBe(true);
      expect(mockVisualizationState._collapseContainerForCoordinator).toHaveBeenCalledWith("container1");
      expect(mockVisualizationState._expandContainerForCoordinator).not.toHaveBeenCalled();
    });
    ...
    it("should handle debouncing", (done) => {
      ...
      // Wait for debounce
      setTimeout(() => {
        expect(mockVisualizationState._expandContainerForCoordinator).toHaveBeenCalledTimes(1);
        done();
      }, 200);
    });
  });
  ...
  describe("batchContainerOperationsImperatively", () => {
    it("should execute multiple operations", () => {
      ...
      expect(mockVisualizationState._expandContainerForCoordinator).toHaveBeenCalledWith("container1");
      expect(mockVisualizationState._collapseContainerForCoordinator).toHaveBeenCalledWith("container2");
      expect(mockVisualizationState._expandContainerForCoordinator).toHaveBeenCalledWith("container3");
    });
    ...
    it("should handle missing containers gracefully", () => {
      ...
      const result = batchContainerOperationsImperatively({
        operations: [
          { containerId: "nonexistent", operation: "expand" }
        ],
        visualizationState: mockVisualizationState
      });
