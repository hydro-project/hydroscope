...
describe("AutoFit Utilities", () => {
  describe("shouldExecuteFitView", () => {
    it("should return true when force is true regardless of autoFitEnabled", () => {
      expect(shouldExecuteFitView({ autoFitEnabled: false, force: true })).toBe(
        true,
      );
      expect(shouldExecuteFitView({ autoFitEnabled: true, force: true })).toBe(
        true,
      );
    });

    it("should respect autoFitEnabled when force is false", () => {
      expect(shouldExecuteFitView({ autoFitEnabled: true, force: false })).toBe(
        true,
      );
      expect(
        shouldExecuteFitView({ autoFitEnabled: false, force: false }),
      ).toBe(false);
    });
  ...
  describe("Integration scenarios", () => {
    ...
    it("should handle style changes correctly (respect setting)", () => {
      ...
      const enabledOptions = createAutoFitOptions(
        AutoFitScenarios.STYLE_CHANGE,
        true,
      );
      ...
      const disabledOptions = createAutoFitOptions(
        AutoFitScenarios.STYLE_CHANGE,
        false,
      );
