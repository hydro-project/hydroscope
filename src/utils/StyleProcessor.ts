...
export function processSemanticTags(
  ...
  if (styleConfig?.semanticMappings) {
    return processWithSemanticMappings(semanticTags, styleConfig, originalLabel, elementType);
  }

  // If we have property mappings, use them (legacy support)
  if (styleConfig?.propertyMappings) {
    return processWithPropertyMappings(semanticTags, styleConfig, originalLabel, elementType);
  }
...
function processWithSemanticMappings(
  ...
  if (Object.keys(styleSettings).length > 0) {
    return convertStyleSettingsToVisual(styleSettings, appliedTags, originalLabel, elementType);
  }
...
function processWithPropertyMappings(
  ...
  return {
    ...
    animated,
    label: labels.length > 0 ? labels.join('') : createTagLabel(appliedTags, originalLabel),
    appliedTags,
  };
}
...
function convertStyleSettingsToVisual(
  ...
  if (halo && halo !== 'none') {
    const haloColor = HALO_COLOR_MAPPINGS[halo as keyof typeof HALO_COLOR_MAPPINGS];
    if (haloColor) {
 ...
 */
function getDefaultStyle(elementType: 'node' | 'edge' = 'edge'): ProcessedStyle {
  if (elementType === 'node') {
    return {
      style: {
 ...
 */
function createTagLabel(appliedTags: string[], originalLabel?: string): string | undefined {
