...
export const SearchIntegration: React.FC<SearchIntegrationProps> = ({
  ...
  const findContainersToExpand = useCallback((results: SearchResult[]): string[] => {
    ...
    for (const result of results) {
      if (result.type === 'node') {
        ...
        if (nodeContainer) {
          ...
          const ancestors = [nodeContainer, ...visualizationState.getContainerAncestors(nodeContainer)];
        ...
        const parentContainers = visualizationState.getContainerAncestors(result.id);
      ...
      if (results.length > 0) {
        ...
        for (const containerId of containersToExpand) {
          try {
            ...
            console.error(`Error expanding container ${containerId} for search:`, error);
          }
        }
  ...
  }, [visualizationState, findContainersToExpand, onContainerExpansion, onLayoutUpdate]);
  ...
  const handleResultSelect = useCallback((result: SearchResult) => {
    ...
    for (const containerId of containersToExpand) {
      try {
        ...
        console.error(`Error expanding container ${containerId} for result selection:`, error);
      }
    }
  ...
  }, [visualizationState, findContainersToExpand, onContainerExpansion, onLayoutUpdate, onSearchResultSelect]);
