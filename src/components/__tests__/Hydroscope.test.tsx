...
vi.mock('../HydroscopeCore', () => ({
  HydroscopeCore: ({ data, onError }: any) => {
    ...
    return <div data-testid="hydroscope-core">HydroscopeCore with {data.nodes?.length || 0} nodes</div>;
  }
}));

// Mock the panels
vi.mock('../panels/InfoPanel', () => ({
  InfoPanel: ({ open }: any) => open ? <div data-testid="info-panel">InfoPanel</div> : null
}));

vi.mock('../panels/StyleTuner', () => ({
  StyleTuner: ({ open }: any) => open ? <div data-testid="style-tuner">StyleTuner</div> : null
}));
