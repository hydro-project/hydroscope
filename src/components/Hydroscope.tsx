...
import { HydroscopeCore, type HydroscopeCoreProps, type HydroscopeCoreHandle } from "./HydroscopeCore.js";
...
export const Hydroscope = memo<HydroscopeProps>(({
  ...
  // Track current grouping separately
  const [currentGrouping, setCurrentGrouping] = useState<string | undefined>(
    data?.nodeAssignments ? Object.keys(data.nodeAssignments)[0] : undefined
  );
  ...
  useEffect(() => {
    if (data !== state.data) {
      ...
      // Also update current grouping when data changes
      setCurrentGrouping(data?.nodeAssignments ? Object.keys(data.nodeAssignments)[0] : undefined);
    }
  }, [data, state.data]);
          ...
          <ReactFlowProvider>
            <div style={{ position: 'relative', height: '100%', width: '100%' }}>
              ...
              <HydroscopeCore
                ...
                height="100%"
                width="100%"
                showControls={false} // Controls are handled by CustomControls component
              ...
              {showInfoPanel && (
                <InfoPanel
                  ...
                  onGroupingChange={(groupingId) => {
                    ...
                    console.log('🔄 Current grouping before change:', currentGrouping);
                    
                    // Update the current grouping state
                    // HydroscopeCore will automatically re-parse when defaultHierarchyChoice changes
                    setCurrentGrouping(groupingId);
                    
                    console.log('✅ Grouping updated, HydroscopeCore will re-parse automatically');
                    console.log('🔄 New grouping will be:', groupingId);
                  }}
              ...
              {(showInfoPanel || showStylePanel) && (
                ...
                >
                  {showInfoPanel && (
                    <button
                      onClick={() => setState(prev => ({ ...prev, infoPanelOpen: !prev.infoPanelOpen }))}
                      style={{
                        width: "40px",
                        height: "40px",
                  ...
                  {showStylePanel && (
                    <button
                      onClick={() => setState(prev => ({ ...prev, stylePanelOpen: !prev.stylePanelOpen }))}
                      style={{
                        width: "40px",
                        height: "40px",
