...
function EdgeStyleLegendInner({
  ...
  const generateVisualSample = (styleSettings: Record<string, string | number>) => {
    ...
    const haloColor =
      halo && halo !== 'none' ? haloColors[halo as keyof typeof haloColors] : undefined;
  ...
  const renderBooleanPairBoxes = () => {
    ...
    return edgeStyleConfig.booleanPropertyPairs.map((pairConfig, index) => {
      ...
      const defaultSample =
        EDGE_STYLE_SAMPLES[pairConfig.defaultStyle as keyof typeof EDGE_STYLE_SAMPLES];
      ...
      return (
        <div key={`pair-${index}`} style={styles.pairBoxStyle}>
          ...
          <div style={styles.contentStyle}>
            <div style={styles.edgeRowStyle}>
              <div style={styles.sampleStyle}>{defaultSample || <span>—</span>}</div>
              <span style={styles.labelStyle}>{defaultProp}</span>
            </div>

            <div style={styles.edgeRowStyle}>
              <div style={styles.sampleStyle}>{altSample || <span>- -</span>}</div>
