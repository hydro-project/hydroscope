...
export const SearchControls = forwardRef<SearchControlsRef, Props>(
  (
    ...
    const navigateToResultIndex = useCallback((index: number, direction: "prev" | "next" = "next") => {
      try {
        ...
        console.error(`[SearchControls] Navigation failed for result ${index}:`, error);
        ...
        if (result) {
          searchNavigationErrorHandler.handleNavigationFailure(
            ...
            console.error(`[SearchControls] Error handler failed:`, handlerError);
          });
        }
    ...
    }, [matches, onNavigate, onResultNavigation, searchResults, announceToScreenReader]);
    ...
    }, [query, searchableItems, announceResults, announceToScreenReader, addToHistory, onSearch]);
    ...
    useEffect(() => {
      if (currentSearchIndex !== currentIndex && currentSearchIndex < matches.length) {
      ...
      return (
        <List.Item
          ...
          style={{
            ...
            borderLeft: isCurrentResult ? "3px solid #1890ff" : "3px solid transparent",
          }}
          onClick={() => {
    ...
    return (
      ...
      >
          ...
          <Input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Enter" && e.shiftKey) {
                ...
                e.preventDefault();
                navigate("prev");
              } else if (e.key === "Tab" && showResultsList && matches.length > 0) {
                e.preventDefault();
            ...
            onBlur={(e) => {
              ...
              if (!resultsListRef.current?.contains(e.relatedTarget as Node)) {
        ...
        {(matches.length > 0 || (searchResults && searchResults.length > 0)) && (showHierarchyPath || showElementType) && (
      ...
      {showResultsList && (matches.length > 0 || (searchResults && searchResults.length > 0)) && searchResults && (
        <div
          ref={resultsListRef}
          style={{
            position: "absolute",
          ...
          onKeyDown={(e) => {
            if (e.key === "Escape") {
              ...
              e.preventDefault();
              const nextIndex = Math.min(focusedResultIndex + 1, matches.length - 1);
