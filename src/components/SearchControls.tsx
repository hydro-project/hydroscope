...
export const SearchControls = forwardRef<SearchControlsRef, Props>(({
  ...
  const navigate = (dir: 'prev' | 'next') => {
    ...
    const idx = dir === 'next' ? (currentIndex + 1) % matches.length : (currentIndex - 1 + matches.length) % matches.length;
    setCurrentIndex(idx);
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: compact ? 6 : 10 }}>
      ...
      <span style={{ minWidth: 52, textAlign: 'center', fontSize: 12, color: '#666' }}>{countText}</span>
      <Tooltip title="Previous match">
        <Button size={compact ? 'small' : 'middle'} onClick={() => navigate('prev')} disabled={!matches.length}>↑</Button>
      </Tooltip>
      <Tooltip title="Next match">
        <Button size={compact ? 'small' : 'middle'} onClick={() => navigate('next')} disabled={!matches.length}>↓</Button>
      </Tooltip>
      <Tooltip title="Clear">
        <Button size={compact ? 'small' : 'middle'} onClick={clearAll} disabled={!query}>✕</Button>
      </Tooltip>
    </div>
  );
});
