...
const HydroscopeCoreInternal = forwardRef<HydroscopeCoreHandle, HydroscopeCoreProps>(({
  ...
  useEffect(() => {
    try {
      ...
      // Create JSONParser
      console.log('[HydroscopeCore] Creating JSONParser with defaultHierarchyChoice:', defaultHierarchyChoice);
      jsonParserRef.current = new JSONParser({
        defaultHierarchyChoice: defaultHierarchyChoice,
        debug: false,
  ...
  }, [initialLayoutAlgorithm, defaultHierarchyChoice]);
  ...
  // Track if we've already processed this data with this hierarchy choice to prevent infinite loops
  const processedDataRef = useRef<{ data: HydroscopeData | null; hierarchyChoice: string | undefined }>({ 
    data: null, 
    hierarchyChoice: undefined 
  });
  ...
  useEffect(() => {
    ...
    // Prevent re-processing the same data with the same hierarchy choice
    console.log('[HydroscopeCore] Data parsing check:', {
      hasData: !!data,
      currentHierarchy: processedDataRef.current.hierarchyChoice,
      newHierarchy: defaultHierarchyChoice,
      sameData: processedDataRef.current.data === data,
      sameHierarchy: processedDataRef.current.hierarchyChoice === defaultHierarchyChoice
    });
    
    if (processedDataRef.current.data === data && processedDataRef.current.hierarchyChoice === defaultHierarchyChoice) {
      console.log('[HydroscopeCore] Skipping re-parse: same data and hierarchy');
      return;
    }
    ...
    const parseAndRender = async () => {
      try {
        ...
        if (state.asyncCoordinator) {
          ...
          interactionHandlerRef.current.handleContainerClick = async (containerId: string, _position?: { x: number; y: number }) => {
            // Skip container interactions in readOnly mode
            if (readOnly) {
              console.log('[HydroscopeCore] Container interaction disabled in readOnly mode');
              return;
            }
  ...
  const handleCollapse = useCallback(async (containerId: string) => {
    if (!state.visualizationState || !state.asyncCoordinator) {
      console.warn('[HydroscopeCore] Cannot collapse container - missing dependencies');
      return;
    }

    const container = state.visualizationState.getContainer(containerId);
    if (!container) {
      console.warn(`[HydroscopeCore] Cannot collapse - container ${containerId} not found`);
      return;
    }

    if (container.collapsed) {
      console.log(`[HydroscopeCore] Container ${containerId} is already collapsed`);
      return;
    }

    const initialCollapsed = container.collapsed;
    try {
      console.log(`[HydroscopeCore] Starting collapse operation for container ${containerId}`);
      ...
      console.error(`[HydroscopeCore] Error collapsing container ${containerId}:`, error);
      ...
      try {
        ...
        console.error(`[HydroscopeCore] Rollback failed for container ${containerId}:`, rollbackError);
      }
      
      handleError(error as Error, `collapse container ${containerId}`);
    }
  }, [state.visualizationState, state.asyncCoordinator, updateReactFlowDataWithState, handleError, onContainerCollapse]);

  const handleExpand = useCallback(async (containerId: string) => {
    if (!state.visualizationState || !state.asyncCoordinator) {
      console.warn('[HydroscopeCore] Cannot expand container - missing dependencies');
      return;
    }

    const container = state.visualizationState.getContainer(containerId);
    if (!container) {
      console.warn(`[HydroscopeCore] Cannot expand - container ${containerId} not found`);
      return;
    }

    if (!container.collapsed) {
      console.log(`[HydroscopeCore] Container ${containerId} is already expanded`);
      return;
    }

    const initialCollapsed = container.collapsed;
    try {
      console.log(`[HydroscopeCore] Starting expand operation for container ${containerId}`);
      ...
      console.error(`[HydroscopeCore] Error expanding container ${containerId}:`, error);
  ...
  }, [state.visualizationState, state.asyncCoordinator, updateReactFlowDataWithState, handleError, onContainerExpand]);

  const handleToggle = useCallback(async (containerId: string) => {
    if (!state.visualizationState) {
      console.warn('[HydroscopeCore] Cannot toggle container - missing VisualizationState');
      return;
    }

    const container = state.visualizationState.getContainer(containerId);
    if (!container) {
      console.warn(`[HydroscopeCore] Cannot toggle - container ${containerId} not found`);
      return;
    }

    console.log(`[HydroscopeCore] Toggling container ${containerId} from ${container.collapsed ? 'collapsed' : 'expanded'}`);
  ...
  useImperativeHandle(ref, () => ({
    collapseAll: readOnly ? async () => console.warn('[HydroscopeCore] collapseAll disabled in readOnly mode') : handleCollapseAll,
    expandAll: readOnly ? async () => console.warn('[HydroscopeCore] expandAll disabled in readOnly mode') : handleExpandAll,
