...
export interface ContainerControlsProps {
  ...
  onOperationComplete?: (operation: 'expand' | 'collapse', containerId?: string) => void;
...
export const ContainerControls: React.FC<ContainerControlsProps> = ({
  ...
  const handleExpandAll = useCallback(async () => {
    ...
    try {
      ...
      const collapsedContainers = visualizationState.visibleContainers
  ...
  }, [disabled, state.isExpanding, visualizationState, asyncCoordinator, onOperationComplete, handleError]);
  ...
  const handleCollapseAll = useCallback(async () => {
    ...
    try {
      ...
      const expandedContainers = visualizationState.visibleContainers
        .filter(container => !container.collapsed);
  ...
  }, [disabled, state.isCollapsing, visualizationState, asyncCoordinator, onOperationComplete, handleError]);

  // Expand specific container
  const handleExpandContainer = useCallback(async (containerId: string) => {
    if (disabled || state.expandingContainers.has(containerId)) return;

    try {
      setState(prevState => ({
        ...prevState,
        expandingContainers: new Set([...prevState.expandingContainers, containerId]),
        lastError: undefined,
        operationCount: prevState.operationCount + 1
      }));
  ...
  }, [disabled, state.expandingContainers, asyncCoordinator, visualizationState, onOperationComplete, handleError]);

  // Collapse specific container
  const handleCollapseContainer = useCallback(async (containerId: string) => {
    if (disabled || state.collapsingContainers.has(containerId)) return;

    try {
      setState(prevState => ({
        ...prevState,
        collapsingContainers: new Set([...prevState.collapsingContainers, containerId]),
        lastError: undefined,
        operationCount: prevState.operationCount + 1
      }));

      await asyncCoordinator.collapseContainer(containerId, visualizationState, {
        triggerLayout: true
      });

      setState(prevState => {
        const newCollapsingContainers = new Set(prevState.collapsingContainers);
  ...
  }, [disabled, state.collapsingContainers, asyncCoordinator, visualizationState, onOperationComplete, handleError]);
    ...
    <div className={`hydroscope-container-controls ${className}`}>
      ...
      <div className="container-controls-main">
        <button
          ...
          disabled={disabled || state.isExpanding || containerStats.collapsed === 0}
          className={`expand-all-btn ${state.isExpanding ? 'loading' : ''}`}
          title={`Expand all containers (${containerStats.collapsed} collapsed)`}
        >
          {state.isExpanding ? 'Expanding...' : `Expand All (${containerStats.collapsed})`}
        </button>

        <button
          onClick={handleCollapseAll}
          disabled={disabled || state.isCollapsing || containerStats.expanded === 0}
          className={`collapse-all-btn ${state.isCollapsing ? 'loading' : ''}`}
          title={`Collapse all containers (${containerStats.expanded} expanded)`}
        >
          {state.isCollapsing ? 'Collapsing...' : `Collapse All (${containerStats.expanded})`}
        </button>
      </div>
      ...
      {showFeedback && (
        <div className="container-controls-status">
          <div className="container-stats">
            ...
            <span className="stat">
              Expanded: {containerStats.expanded}
            </span>
            <span className="stat">
              Collapsed: {containerStats.collapsed}
            </span>
          </div>

          {/* Loading States */}
          {(state.isExpanding || state.isCollapsing || 
...
export const IndividualContainerControl: React.FC<IndividualContainerControlProps> = ({
  container,
  visualizationState,
  asyncCoordinator,
  onOperationComplete,
  ...
  const handleToggle = useCallback(async () => {
    ...
    try {
      ...
      if (container.collapsed) {
        await asyncCoordinator.expandContainer(container.id, visualizationState, {
          triggerLayout: true
        });
        if (onOperationComplete) {
        ...
        await asyncCoordinator.collapseContainer(container.id, visualizationState, {
          triggerLayout: true
        });
        if (onOperationComplete) {
  ...
  }, [disabled, isOperating, container, asyncCoordinator, visualizationState, onOperationComplete, onError]);
...
export const useContainerControls = (
  ...
  const toggleContainer = useCallback(async (containerId: string) => {
